...
const wellKnownName = 'org.freedesktop.NetworkManager';
const iface = 'org.freedesktop.NetworkManager.Settings.Connection';

function getSettings(objectPath) {
...
export async function getZone(objectPath) {
    const settings = await getSettings(objectPath);
...
export async function setZone(objectPath, zone) {
    // get existing settings
    const settings = await getSettings(objectPath);
    // create a new variant with the correct zone
    const parameters = createGvariantTuple(settings, zone);
    // replace zone
    return new Promise((resolve, reject) => {
        Gio.DBus.system.call(
            wellKnownName,
...
function createGvariantTuple(dbusResult, newZone) {
